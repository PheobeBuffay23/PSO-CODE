...
public class PSO {
    public static void main(String[] args) {
        ...
        double[] personalbestX = position.clone(); // maintaing an array to store the personal best position of a
                                                  // particle( which is initially equal to particle initial positions )
        double[] personalbestY = position.clone();               
        double globalbestX = position[0];
        double globalbestY = position[0];
       ...
       //int maxiterations=Integer.parseInt(args[0]);
        ...
        double globalbestvalueY = 0;
 
        for (int i = 0; i < maxiterations; i++) {
 
            for (int j = 0; j < 5; j++) {
                double positionfunction = ob.ObjectiveFunction(positionX[j]); // positionfunction store
                double positionfunction = ob.ObjectiveFunction(positionY[j]);// position[j]*positioon[j](multiple points
                                                                             // to give value)
                double personalbestfunction = ob.ObjectiveFunction(personalbestX[j]); // personalbestfunction store
                double personalbestfunction = ob.ObjectiveFunction(personalbestY[j]);// personalbest[j]*personalbest[j]
                                                                                     // (multiple points to give value)
                globalbestvalue = ob.ObjectiveFunction(globalbestX); // globalbestvalue store globalbest*globalbest(ie.
                globalbestvalue = ob.ObjectiveFunction(globalbestY);   // initially equal to position of first element)
                ...
                if (personalbestfunction < globalbestvalue) { /*
                                                               ...
                                                               * elements
                                                               */
                    globalbest = personalbestX[j];
                    globalbest = personalbestY[j];
        ...
        System.out.println("the value of gbest id " + globalbestvalueX);
        System.out.println("the value of gbest id " + globalbestvalueY);
